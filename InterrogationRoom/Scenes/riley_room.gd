...
func _process(delta: float) -> void:
	...
	if !$Laptop.visible && !$RileyTalkingScene.visible:
		enable = true
	if $RileyTalkingScene.visible:
		$Label.hide()
		enable = false
...
func RileyIntro() -> void:
	
	$Label.hide()
	$Node2D/Riley/intro.hide()
	$RileyTalkingScene.show()
	
	#$Node2D/greg/question.texture_hover = hover_question
	$Node2D/Riley/question.show()
	emit_signal("intro")	


func _on_Riley_talking_scene_done() -> void:
	$RileyTalkingScene.hide()
	$Label.show()
