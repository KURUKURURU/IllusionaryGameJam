...
func _process(delta: float) -> void:
	...
	if !$Laptop.visible && !$StephTalkingScene.visible:
		enable = true
	if $StephTalkingScene.visible:
		$Label.hide()
		enable = false
...
func Leave() -> void:
	if Global.steph_saved:
		...
		get_tree().change_scene_to_file("uid://lk7mvn3lujw3")
	else:
		$bing.play()


func StephIntro() -> void:
	
	$Label.hide()
	$Node2D/Steph/intro.hide()
	$StephTalkingScene.show()
	
	#$Node2D/greg/question.texture_hover = hover_question
	$Node2D/Steph/question.show()
	emit_signal("intro")	


func _on_Steph_talking_scene_done() -> void:
	$StephTalkingScene.hide()
	$Label.show()
